...
window.logout = async () => {
  ...
  state.role = null;
  state.user = null; // Limpiar datos de usuario
  state.path = '/';
  renderApp();
};
...
window.handleLogin = async () => {
  ...
  try {
    ...
    const { data: allowed, error: errAllowed } = await supabaseClient
        .from('perfiles_permitidos')
        .select('email, rol, nombre')
        .eq('email', email)
        .maybeSingle();

    if (errAllowed || !allowed) {
        errorMsg.innerText = "Este correo no está registrado en el padrón autorizado.";
    ...
    try {
        ...
        if (!signInError) {
            ...
            errorMsg.innerText = "Acceso correcto. Sincronizando perfil...";
...
function renderSetupScreen() {
    ...
    if (currentStep === 2) { // Mi escuela ya está registrada
        return `
        <div style="display:flex; justify-content:center; align-items:center; min-height:100vh; background:var(--secondary-bg); padding:20px;">
          <div class="card shadow-lg" style="width:100%; max-width:500px; padding:40px; border-top: 6px solid var(--success); text-align:center; animation: slideInRight 0.4s;">
             ...
             <div class="form-group">
                <input type="text" id="checkEscuelaName" class="form-input" placeholder="ESCRIBA EL NOMBRE EN MAYÚSCULAS" style="text-align:center; font-weight:bold; height:60px; font-size:1.1rem;" oninput="this.value = this.value.toUpperCase()">
             </div>
             
             <button class="btn btn-success" style="width:100%; height:50px;" onclick="window.validarEscuelaYaRegistrada()">
...
window.validarEscuelaYaRegistrada = async () => {
    const btn = event?.currentTarget;
    const inputName = document.getElementById('checkEscuelaName').value.trim().toUpperCase();
    if(!inputName) return alert("Escriba el nombre de su escuela.");

    if(btn) btn.disabled = true;
    try {
        const { data, error } = await supabaseClient.from('config_institucion').select('nombre_escuela').eq('id', 1).maybeSingle();
        if(data && data.nombre_escuela.toUpperCase() === inputName) {
            window.showToast("Escuela Validada. Cargando portal...", "success");
            state.schoolConfigured = true;
            CONFIG.schoolName = data.nombre_escuela;
            renderApp();
        } else {
            alert("No encontramos ese nombre en nuestro sistema o aún no ha sido registrada por tu Director. Verifica que esté en MAYÚSCULAS.");
        }
    ...
    try {
        ...
        if(ownerId) {
            await supaAdmin.from('perfiles').upsert({
                id: ownerId,
                nombre: dir,
                rol: 'admin', // El creador es Admin/Director por defecto
        ...
        window.showToast("¡Felicidades! Tu plantel ha sido creado exitosamente.", "success");
        renderApp();
    } catch(e) { 
        alert("Error en Setup: " + e.message); 
        if(btn) {
            btn.disabled = false;
            btn.innerHTML = '<i class="fa-solid fa-rocket"></i> Registrar Plantel y Acceder';
        }
    }
};
...
window.checkSchoolSetup = async () => {
    ...
    try {
        // 1. Ver si hay un usuario logueado con plantel asignado
        const { data: { session } } = await supabaseClient.auth.getSession();
        
        if(session && session.user) {
            const { data: profile } = await supabaseClient.from('perfiles')
                .select('plantel_id, planteles(*)')
                .eq('id', session.user.id)
                .maybeSingle();
            
            if(profile && profile.plantel_id) {
                state.plantelId = profile.plantel_id;
                CONFIG.schoolName = profile.planteles.nombre;
                state.schoolConfigured = true;
                renderApp();
                return;
            }
        ...
        // 2. Si no hay sesión o no hay plantel, buscar si hay al menos una escuela (Legacy Fallback)
        const { data: globalCheck } = await supabaseClient.from('planteles').select('id').limit(1);
        if(!globalCheck || globalCheck.length === 0) {
            state.schoolConfigured = false;
        } else {
            // Si hay escuelas pero el usuario no tiene una, lo mandamos al login (Role Selector)
            state.schoolConfigured = true;
        }
        renderApp();
    } catch(e) { 
    ...
    <div class="role-selector-view" style="display:flex; justify-content:center; align-items:center; min-height:100vh; background:var(--page-bg); padding:20px;">
      <div class="card shadow-lg" style="width:100%; max-width:420px; padding:40px; border-radius:30px; text-align:center; animation: fadeInDown 0.5s ease-out;">
        ...
        <div class="form-group">
          ...
          <input type="email" id="fb-email" class="form-input" placeholder="ejemplo@escuela.edu.mx">
        </div>
        ...
        <div style="margin-top:20px; padding:12px; border: 1.5px solid #e2e8f0; border-radius:12px; background:#f8fafc; cursor:pointer;" onclick="document.getElementById('fb-email').value='zlagustin10@gmail.com'; window.showToast('Acceso Maestro Activado 👑', 'info');">
...
function renderSidebar() {
  ...
  const roleNames = { admin: 'Administrativo', directivo: 'Directivo', maestro: 'Maestro', apoyo: 'Trabajo Social', alumno: 'Estudiante', administrador: 'Administrativo' };

  const userName = state.userName || state.user?.user_metadata?.nombre || state.user?.email || 'Usuario';
...
function renderApp() {
  ...
  try {
    if (!state.role) {
      ...
      const pageContent = renderPage(state.path);
