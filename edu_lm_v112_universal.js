    ...
    <div class="card">
       <div style="display:flex; justify-content:space-between; align-items:flex-end; margin-bottom: 24px; flex-wrap:wrap; gap:12px;">
          ...
          <div style="display:flex; gap:8px; flex-wrap:wrap;">
             <button class="btn btn-outline" style="border-color:var(--text-muted); color:var(--text-main)" onclick="window.exportarRejillaBlancoCSV()" title="Descargar plantilla CSV en blanco (20 columnas)"><i class="fa-solid fa-file-csv"></i> CSV Vacío</button>
             <button class="btn btn-outline" style="border-color:#6366f1; color:#6366f1" onclick="window.imprimirLista(true)" title="Imprimir plantilla oficial en blanco (15 columnas) con membrete"><i class="fa-solid fa-print"></i> Imprimir Vacía</button>
             ...
             <button class="btn btn-outline" style="border-color:var(--primary); color:var(--primary)" onclick="window.imprimirLista(false)"><i class="fa-solid fa-print"></i> Imprimir Lista</button>
          </div>
       </div>
...
window.loadApoyoRiesgoData = async () => {
    ...
    const gradoSel = document.getElementById('riesgoGradoSel').value;
...
function renderApoyoBitacora() {
    ...
    <div class="page-header" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:15px;">
      ...
      <div style="display:flex; gap:10px;">
        ...
        <button class="btn btn-outline" style="border-color:var(--primary); color:var(--primary); display:flex; align-items:center; gap:8px;" onclick="window.imprimirApoyoBitacora()">
            <i class="fa-solid fa-print"></i> Imprimir Mis Hechos
        </button>
      </div>
    </div>
...
window.eliminarTramiteEntregado = async (tramiteId) => {
    if(!confirm("⚠️ ¿Estás seguro de anular esta entrega?\n\nEl trámite saldrá del historial y regresará a la bandeja de 'Solicitudes Pendientes' para que puedas volver a subir el documento correcto.")) return;
    ...
    try {
        ...
        window.showToast("Entrega anulada. El trámite regresó a Solicitudes Pendientes.", "success");
        if(window.switchTramiteView) window.switchTramiteView('historial');
    } catch(err) {
        console.error(err);
        window.showToast("Error al anular entrega: " + err.message, "error");
    }
};
...
window.imprimirApoyoBitacora = async () => {
    const fecha = document.getElementById('fechaBitacoraApoyo')?.value || new Date().toLocaleDateString('en-CA');
    try {
        let query = supabaseClient
        ...
        if (state.role === 'apoyo') {
            query = query.eq('perfil_id', state.user.id);
        }
...
window.imprimirLista = async (esVacia = false) => {
    const tabla = document.querySelector(".risk-table");
    const tbody = document.getElementById('listaMaestroAlumnos');
    const statsCont = document.getElementById('statsListaMaestro');
...
window.imprimirBitacoraGeneral = async () => {
